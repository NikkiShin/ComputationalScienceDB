...
public class MakeJSONDocumentToMongoDB {
	public MakeJSONDocumentToMongoDB(String[] args) {
		...
		String[] d_input = {"Grid", "Umach", "AOA", "RE", "IVISC", "rho_inf", "t_inf", "p_inf", "t_wall", "intensity", "f_func", "f_order", "liminter" };
		...
		try {
			...
			for(int i=0; i<5; i++)
				doc_aerodynamic.put(d_output[i], args[i+13]);
			...
			for(int i=2; i<8; i++){
				String s = String.format("%03d", i);
				doc_surface.put("sur" + s , args[i+32]);
			}
